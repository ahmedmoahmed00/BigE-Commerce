import { useState } from "react";
import InputSearch from "../../features/admin/components/shared/InputSearch";
import { FaPlus } from "react-icons/fa6";
import TabelAdminCategories from "../../features/admin/components/categories/TabelAdminCategories";
import type { CategoryDetails } from "../../features/admin/types";
import ManageCategoryModal from "../../features/admin/components/categories/ManageCategoryModal";

function AdminCategories() {
  const [showModal, setShowModel] = useState<boolean>(false);

  return (
    <div>
      <header className="flex flex-col lg:flex-row gap-2 items-center justify-between ">
        <h2 className="font-semibold mr-auto">Categories Management</h2>
        <button
          onClick={() => setShowModel(true)}
          className="flex items-center justify-center w-full lg:w-fit gap-2 cursor-pointer bg-secondary text-primary px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors"
        >
          <FaPlus />
          Add Category
        </button>
      </header>
      <main className="mt-6 space-y-6">
        <div>
          <InputSearch />
        </div>
        <div>
          <TabelAdminCategories />
        </div>
        {showModal && (
          <ManageCategoryModal
            onClose={() => setShowModel(false)}
            categoryToEdit={{} as CategoryDetails}
          />
        )}
      </main>
    </div>
  );
}

export default AdminCategories;
